...
pub fn list_all_ingredients(conn: &Connection) -> Result<Vec<Ingredient>> {
    ...
    let mut stmt = conn.prepare("SELECT name, category, unit FROM ingredients")?;
    ...
    let ingredients = stmt
        .query_map([], |row| {
            Ok(Ingredient {
                name: row.get(0)?,
                category: Ingredient::convert_to_category(row.get(1)?),
                unit: Ingredient::convert_to_unit(row.get(2)?),
            })
        })?
